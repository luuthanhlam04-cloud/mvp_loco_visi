...
export function ItineraryTimeline() {
  ...
  useEffect(() => {
    if (activeActivityIndex !== undefined) {
  ...
  return (
    <div className="flex w-full h-screen bg-indochine-bg overflow-hidden p-4 gap-6">
      {/* Left Column: Timeline & Input */}
      <div className="w-full lg:w-[40%] flex flex-col h-full bg-white rounded-3xl shadow-xl border border-indochine-yellow-dark/20 overflow-hidden relative z-10">
        
        {/* Header */}
        <div className="p-6 border-b border-indochine-bg bg-white/80 backdrop-blur z-20">
          <div className="flex items-center gap-2 mb-2">
            <Sparkles className="text-indochine-green-light" size={24} />
            <h1 className="text-2xl font-bold text-indochine-dark">LOCO AI</h1>
          </div>
          <p className="text-sm text-gray-500">Dệt nên lịch trình hoàn hảo của riêng bạn</p>
        </div>

        {/* Timeline Area */}
        <div 
          ref={scrollRef}
          className="flex-grow overflow-y-auto no-scrollbar p-6 space-y-6"
        >
          {!object && !isLoading && (
          ...
          <Reorder.Group axis="y" values={validActivities} onReorder={setLocalActivities} className="space-y-6">
            {validActivities.map((activity, index) => {
              ...
              return (
                <ActivityCard 
                  ...
                  onClick={() => setActivePlaceId(activity.place_id)}
        ...
        {/* Input Area */}
        <div className="p-4 bg-white border-t border-indochine-bg relative z-20">
          <form onSubmit={handleSubmit} className="relative">
            <input
              ...
              className="w-full pl-6 pr-14 py-4 rounded-full bg-indochine-bg border border-indochine-yellow-dark/30 focus:outline-none focus:ring-2 focus:ring-indochine-green-light/50 focus:bg-white transition-all text-indochine-dark placeholder-gray-400 shadow-sm"
              disabled={isLoading}
            />
            <button
              type="submit"
      ...
      {/* Right Column: Map */}
      <div className="hidden lg:block lg:w-[60%] h-full rounded-3xl overflow-hidden relative">
        <MapComponent 
